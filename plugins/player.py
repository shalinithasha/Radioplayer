...
async def play_track(client, m: Message):
    ...
    if m.audio:
        ...
        await client.send_message(text="ඔබ මට වාදනය කිරීමට කිසිවක් ලබා දුන්නේ නැත. මට ශ්‍රව්‍ය ගොනුවක් එවන්න හෝ ශ්‍රව්‍ය ගොනුවකට පිළිතුරු / වාදනය කරන්න!", chat_id=chat_id)
        if LOG_GROUP:
            await mp.send_playlist()
        return
    if playlist and playlist[-1].audio.file_unique_id \
            == m_audio.audio.file_unique_id:
        await m.reply_text(f"{emoji.ROBOT} **දැනටමත් එකතු කර ඇත!**")
        return
    # add to playlist
    playlist.append(m_audio)
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} **බාගත කිරීම සහ පරිවර්තනය කිරීම...**"
        )
...
async def join_group_call(client, m: Message):
    group_call = mp.group_call
    if group_call.is_connected:
        await m.reply_text(f"{emoji.ROBOT} **දැනටමත් Voice Chat වෙත සම්බන්ධ වී ඇත!**")
        return
    await mp.start_call()
    #await group_call.start(CHAT)
    chat = await client.get_chat(CHAT)
    await m.reply_text(f"{emoji.CHECK_MARK_BUTTON} **Voice Chat සාර්ථකව ඉවත් විය {chat.title} Successfully!**")
...
async def leave_voice_chat(_, m: Message):
    ...
    await m.reply_text(f"{emoji.CROSS_MARK_BUTTON} **Voice Chat සාර්ථකව ඉවත් විය!**")
...
async def list_voice_chat(client, m: Message):
    ...
    if group_call.is_connected:
        ...
        await m.reply_text(emoji.NO_ENTRY
                                   + "**තවම කිසිම Voice Chat එකකට සම්බන්ධ වෙලා නෑ!**")
